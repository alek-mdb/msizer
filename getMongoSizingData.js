  ...
  printInfo("serverInfo", function () {
    ...
    try {
      returnObj = {
        ...
        storageEngine: serverinfo.storageEngine
          && serverinfo.storageEngine.name,
        numCores: db.hostInfo().system
          && db.hostInfo().system.numCores,
        uptimeSec: serverinfo.uptime,
        pageFaults: serverinfo.extra_info
          && serverinfo.extra_info.page_faults,
        ...
        wiredtigerCacheSize: serverinfo.wiredTiger
          && serverinfo.wiredTiger.cache
          && serverinfo.wiredTiger.cache["maximum bytes configured"],
        bytesCurrentlyInCache: serverinfo.wiredTiger
          && serverinfo.wiredTiger.cache
          && serverinfo.wiredTiger.cache["bytes currently in the cache"],
        defaultReadConcern: serverinfo.defaultRWConcern
          && serverinfo.defaultRWConcern.defaultReadConcern,
        defaultWriteConcern: serverinfo.defaultRWConcern
          && serverinfo.defaultRWConcern.defaultWriteConcern,
        indexStats: serverinfo.indexStats,
        transactions: serverinfo.transactions
      };
      returnObj.network = {
        bytesIn: serverinfo.network
          && serverinfo.network.bytesIn,
        bytesOut: serverinfo.network
          && serverinfo.network.bytesOut,
        compression: serverinfo.network
          && serverinfo.network.compression
      };
      returnObj.metrics = {
        opcounters: serverinfo.opcounters,
        document: serverinfo.metrics
          && serverinfo.metrics.document,
        text: serverinfo.metrics
        ...
        regex: serverinfo.metrics
          && serverinfo.metrics.operatorCounters
          && serverinfo.metrics.operatorCounters.match
          && serverinfo.metrics.operatorCounters.match.$regex,
        ttl: serverinfo.metrics
          && serverinfo.metrics.ttl,
        aggStageCounters: serverinfo.metrics
          && serverinfo.metrics.aggStageCounters,
        operatorCounters: serverinfo.metrics
          && serverinfo.metrics.operatorCounters,
      };
      returnObj.wiredTiger = {
        perf: serverinfo.wiredTiger 
          && serverinfo.wiredTiger.perf,
        capacity: serverinfo.wiredTiger 
          && serverinfo.wiredTiger.capacity,
        blockCache: serverinfo.wiredTiger 
          && serverinfo.wiredTiger["block-cache"]
      }
    } catch (err) {
      _errors.push({
        function: "printServerInfo",
...
function printDataInfo() {
    ...
    try{
                    ...
                    if (!/system/.test(collectionInfo.name)) {
                      ...
                      if (collections_counter < _maxCollections){
                        ...
                        collectionstats.push({
                          ...
                          creationString: colstats.wiredTiger
                            && colstats.wiredTiger.creationString,
      ...
      if (collections_counter > _maxCollections){
        throw{
          message: "MaxCollectionsExceededException: There are " +
                    collections_counter +
                    " collections " +
                    "which is above the max allowed of " + _maxCollections + " for this script. No more database and " +
